  ...
  <div class="app-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" fit highlight-current-row empty-text="暂无数据" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="发起用户">
        <template slot-scope="scope">
         {{ userMapJoin[scope.row.uid].mobile }}<br>
         {{ userMapJoin[scope.row.uid].nick }}
        </template>
      </el-table-column>
      <el-table-column label="助力用户">
        <template slot-scope="scope">
         {{ userMapHelp[scope.row.uidHelp].mobile }}<br>
         {{ userMapHelp[scope.row.uidHelp].nick }}
        </template>
      </el-table-column>
      <el-table-column prop="cutPrice" label="金额"></el-table-column>
      <el-table-column prop="dateAdd" label="助力时间" width="160"></el-table-column>
