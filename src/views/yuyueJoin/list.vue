<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-input clearable @keyup.enter.native="fetchData" style="width: 200px;" class="filter-item" placeholder="团队ID" v-model="searchData.teamId">
      </el-input>
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" fit highlight-current-row empty-text="暂无数据" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="状态" align="center">
        <template slot-scope="scope">
          <el-tag size="mini" type="info" v-if="scope.row.status == 0">等待付款</el-tag>
          <el-tag size="mini" type="warning" v-if="scope.row.status == 1">预约成功</el-tag>
          <el-tag size="mini" type="success" v-if="scope.row.status == 2">已服务</el-tag>
          <el-tag size="mini" type="danger" v-if="scope.row.status == 3">取消预约</el-tag>
          <div v-if="scope.row.teamMembers > 0">成员: {{ scope.row.teamMembers }}人</div>
        </template>
      </el-table-column>
...
<script>
import { fetchDataList, successData, delData, cancelData, serveredData, infoData } from '@/api/yuyueJoin'
