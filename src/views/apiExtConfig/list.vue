<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-button size="medium" class="filter-item" @click="handleCreate(0)" type="success">添加文本参数</el-button>
      <el-button size="medium" class="filter-item" @click="handleCreate(1)" type="success">添加开关参数</el-button>
      <el-button size="medium" class="filter-item" @click="handleCreate(2)" type="success">添加文件参数</el-button>
    </div>
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" fit highlight-current-row empty-text="暂无数据" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="内容">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.dateType == 1 && scope.row.value == '1'">开启</el-tag>
          <el-tag type="danger" v-if="scope.row.dateType == 1 && scope.row.value == '0'">关闭</el-tag>
          <span v-if="scope.row.dateType != 1">{{scope.row.value}}</span>
        </template>        
      </el-table-column>
    ...
    <el-dialog :title="pushData.dialogTitle" :visible.sync="pushData.dialogFormVisible" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form :rules="rules" ref="addEditPopForm" :model="pushData" label-position="left" label-width="100px">
        ...
        <el-form-item v-if="pushData.dateType == 0" label="参数值" prop="content" >
          <el-input v-model="pushData.content" type="textarea" :rows="4" clearable></el-input>
        </el-form-item>
...
<script>
import { fetchDataList, getConfigInfo, delData, saveConfig } from '@/api/apiExtConfig'
  ...
  methods: {
    ...
    handleCreateSave(){
      this.$refs['addEditPopForm'].validate((valid) => {
        if (valid) {
          saveConfig(this.pushData).then((res) => {
