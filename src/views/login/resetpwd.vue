...
import { resetpwd, smscode_resetpwd, logout, getInfo } from '@/api/login'
...
export default {
  ...
  methods: {
    ...
    getsmscode() {
      ...
      this.loading = true
      smscode_resetpwd(this.loginForm.username, this.loginForm.imgcode, this.loginForm.picKey).then((res) =>{
        this.loading = false
        if (res.code == 0) {
          Message({
            message: '验证码已发送',
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          resetpwd(this.loginForm.username, this.loginForm.password, this.loginForm.smscode).then((res) => {
            this.loading = false
            if (res.code == 404) {
              this.changeRandom()
              Message({
