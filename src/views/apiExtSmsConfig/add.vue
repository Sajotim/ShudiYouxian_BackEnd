  ...
  data() {
    return {
      ...
      rules: {
        ...
        pwd: [{ required: true, message: "请填写", trigger: "blur" }],
        paixu: [{ required: true, message: "请填写排序", trigger: "blur" }]
        /*type: [
  ...
  methods: {
    ...
    isHidden(type, templateContent) {
      if (type === "alidayu" || type === "aliyun") {
        ...
        this.pushData.templateData = templateContent
          ? templateContent
          : '{code:1234,product:"测试产品"}';
        this.isTemplateId = true;
        this.isSignName = true;
        this.isCode = true;
      } else if (type === "18sms") {
        this.pushData.templateContent = templateContent
          ? templateContent
          : "您的验证码是：${code}";
      } else if (type === "saiyou") {
        this.name = "APPID";
    ...
    handleCreateSave() {
      this.$refs["addEditPopForm"].validate(valid => {
        if (valid) {
          if (this.pushData.type === "aliyun") {
            this.pushData.templateData = this.pushData.templateContent;
          }
