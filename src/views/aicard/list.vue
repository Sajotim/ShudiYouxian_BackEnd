<template>
  <div class="app-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" fit highlight-current-row empty-text="暂无数据" @selection-change="handleSelectionChange">
      <el-table-column label="名片编号" prop="id" />
      ...
      <el-table-column label="操作" width="200px">
        <template slot-scope="scope">
          ...
          <el-button type="text" style="color:red" @click="setToFK(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import { fetchDataList,importFromWX, modifyUser, delData, delUserFriend } from '@/api/apiExtUser'
import { fetchAllUserLevels, saveUserLevelData } from '@/api/apiExtUserLevel'
import { modifyConfig } from '@/api/apiExtConfig'
    ...
    fetchAllUserLevels(1, 1, {
      ...
      if (res.code == 700) {
        saveUserLevelData({
          ...
          _this.searchData.levelId = level.data.id
          _this.fetchData()
        })
      } else {
        _this.userLevelId = res.data.result[0].id
        _this.searchData.levelId = res.data.result[0].id
        _this.fetchData()
      }
    }).catch(e => {
      console.error(e)
    ...
    setToYG(id){
      modifyUser({
        ...
        if (res.code == 30001) {
          ...
          // 设置默认名片
          modifyConfig('DEFAULT_FRIEND_UID', id).then((res) => {
            console.log(res)
          }).catch(e=>{
            console.error(e);
          })
