<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button size="medium" class="filter-item" style="margin-left: 10px;" @click="handleCreate" type="success" icon="el-icon-plus">上传视频</el-button>
    </div>
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" fit highlight-current-row empty-text="暂无数据" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="视频ID/名称/容量">
        <template slot-scope="scope">
          {{ scope.row.videoId }}<br>{{ scope.row.title }}<br>{{ scope.row.sizeStr }}
        </template>
      </el-table-column>
      <el-table-column label="状态" width="80">
        <template slot-scope="scope">
          <el-tag type="info" v-if="scope.row.status == 0">上传中</el-tag>
      ...
      <el-table-column label="视频地址" width="160">
        <template slot-scope="scope">
      ...
      <el-table-column label="上传/更新时间" width="160">
        <template slot-scope="scope">
