    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" fit highlight-current-row empty-text="暂无数据" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="状态">
        <template slot-scope="scope">
          ...
          <el-tag type="danger" v-if="scope.row.status == 1">付款中</el-tag>
        </template>
      </el-table-column>
      ...
      <el-form :rules="rules" ref="addEditPopForm" :model="pushData" label-position="left" label-width="100px">
        ...
        <el-form-item label="状态" prop="status" >
          <el-select v-model="pushData.status" placeholder="请选择">
            ...
            <el-option label="正在付款" value="1"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
import { fetchDataList, infoData, delData, saveData } from '@/api/alipayTransferQrcode'
...
export default {
  ...
  methods: {
    handleAvatarSuccess(res, file) {
      ...
      this.pushData.qrcodePic = res.data.url;
    },
    ...
    handleCreate(){
      ...
      this.pushData.dialogTitle = '添加支付宝收钱码'
