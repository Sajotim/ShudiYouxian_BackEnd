<template>
  <div class="app-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" fit highlight-current-row empty-text="暂无数据" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="拼团ID/商品ID">
        <template slot-scope="scope">
          {{ scope.row.id }}<br>{{ scope.row.goodsId }}
        </template>
      </el-table-column>
      <el-table-column label="成团条件">
        <template slot-scope="scope">
          {{ scope.row.timeoutHours }}小时内<br>满{{ scope.row.numberPersion }}人参与
        </template>
      </el-table-column>
      <el-table-column label="进行中/成功">
        <template slot-scope="scope">
          {{ scope.row.numberDoing }}个团<br>{{ scope.row.numberSucccess }}个团
        </template>
      </el-table-column>
      ...
      <el-table-column prop="refundTypeStr" label="退款至(未成团)" />
      <el-table-column prop="dateEnd" label="截止时间" width="160" />
    ...
    <el-button style='margin-top:20px' size="small" type="danger" @click="delDataMore">批量删除</el-button>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      ...
      this.pushData.dialogTitle = '拼团设置'
